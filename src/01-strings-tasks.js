...
function encodeToRot13(str) {
  const aUp = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ';
  const aLo = 'abcdefghijklmnopqrstuvwxyz';
  let trs = '';
  for (let i = 0; i < str.length; i += 1) {
    if (aLo.includes(str[i])) trs += aLo[(aLo.indexOf(str[i]) + 13) % 26];
    else if (aUp.includes(str[i])) trs += aUp[(aUp.indexOf(str[i]) + 13) % 26];
    else trs += str[i];
  }
  return trs;
}
